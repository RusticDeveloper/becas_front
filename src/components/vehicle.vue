  ...
  <Card>
    ...
    <template #content>
      <div class="flex flex-wrap gap-6">
        ...
        <!-- * Marca -->
        <FormField
          v-slot="$carBrandField, nameId"
          :name="'carBrand' + nameId"
          :resolver="carBrandSchema"
        >
          <FloatLabel variant="on" class="flex-1">
            <InputText type="text" :id="'carBrand' + nameId" variant="filled" />
            <label :for="'carBrand' + nameId">Marca</label>
          </FloatLabel>
          <template v-if="$carBrandField + nameid?.invalid">
            <Message
              v-for="(error, index) of $carBrandField + nameId?.errors"
              :key="index"
              severity="error"
              size="small"
              variant="simple"
        ...
        <FormField v-slot="$carYearField" :name="'carYear' + nameId" :resolver="carYearSchema">
          <FloatLabel variant="on" class="flex-1">
            <InputText type="text" :id="'carYear' + nameId" variant="filled" />
